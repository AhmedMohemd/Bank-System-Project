...
public:
    ...
    Employee(const string& name, int id, const string& password, const string& phone, const string& email, const string& nationaid, double salary)
    ...
    void addClient(const Client& client) {
        ...
        cout << "Client added successfully by employee." << endl;
    }
    ...
    void approveLoanRequest(int clientId, double loanAmount) {
        ...
        if (found) {
            ...
            if (rename("Temp.txt", "Clients.txt") != 0) {
                perror("Error renaming file Clients.txt");
            }
            else {
                cout << "Client file updated successfully." << endl;
            }
            for (auto& client : allClients) {
                if (client.getId() == clientId) {
